"""Backtest realism convention (2026-06-13) — n40 (weekly large-cap rotation).

Guards the convention applied to tools/models/n40/backtest.py:
  ...
  2. Fills at the NEXT bar's open (decision on bar d close -> fill d+1 open);
     ...
     Applies to rotation/entry/profit-take AND the daily fixed stop (detection
     stays on bar d's low via the shared fixed_stop_hit helper).
...
from tools.models.n40 import backtest as B
...
# ---------------- stop fill: detection on level, fill at next open ----------

def test_stop_detection_unchanged_but_fill_is_next_open():
    # fixed_stop_hit (the shared live helper) still detects on bar d's low,
    # while next_open_fill prices the exit at bar d+1's open (gap-realistic).
    from tools.shared.stops import fixed_stop_hit
    op, cl, dates, sym = _panels()
    entry, stop_pct = 115.0, 0.12
    hit, lvl = fixed_stop_hit(entry, 100.0, stop_pct)   # bar 0 low breaches
    assert hit and lvl == pytest.approx(entry * (1 - stop_pct))
    px, fdate = B.next_open_fill(op, cl, dates, sym, 0)
    assert px == 101.0                      # next open, NOT the exact level
    assert fdate == "2026-01-06"
