#!/usr/bin/env python3
"""
Batch Train Raw LSTM Models for Top Stocks

Trains LSTM models for top liquid stocks to populate Raw LSTM predictions.
...
def get_top_stocks(limit=20):
    """Get top liquid stocks by market cap and volume."""
    db = get_database_manager()
    with db.get_session() as session:
        query = text("""
            SELECT symbol, name, market_cap, current_price
            FROM stocks
            WHERE is_active = true
              AND market_cap > 10000  -- Large/mid cap
              AND current_price > 50
              AND current_price < 10000
...
def main():
    ...
    logger.info("█" * 100)
    logger.info("█" + " "*98 + "█")
    logger.info("█" + " "*20 + "BATCH LSTM TRAINING - Top Liquid Stocks" + " "*40 + "█")
    logger.info("█" + " "*98 + "█")
    logger.info("█" * 100)

    # Get top stocks
    logger.info("\nFetching top liquid stocks...")
    stocks = get_top_stocks(limit=20)

    logger.info(f"Selected {len(stocks)} stocks for training:")
    for i, stock in enumerate(stocks, 1):
        logger.info(f"  {i}. {stock['symbol']}: {stock['name']} (MCap: {stock['market_cap']:.0f} Cr)")
    ...
    if success_count > 0:
        logger.info(f"\n✅ Successfully trained {success_count} Raw LSTM models!")
        logger.info("   Models saved to: ml_models/raw_ohlcv_lstm/")
        logger.info("\nNext steps:")
        logger.info("  1. Run: python3 tools/generate_raw_lstm_all_strategies.py")
        logger.info("  2. This will generate predictions for both strategies")
        logger.info("  3. Check UI to see Raw LSTM predictions")
