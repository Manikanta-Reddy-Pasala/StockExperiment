"""PIT-strict walk-forward backtest for n20_daily_30d_mc1_uptrend.

Winning config from 64-config sweep (search target ≥100%/yr CAGR):
  - Universe: top 20 N500 by 20-day ADV (rebuilt per day, lookback-only)
  - Filter: close > 200d SMA (uptrend gate)
  - Signal: rank by 30-day return
  - Position: hold top-1, daily rebalance
  - Exit: rotation only

Reproduces ₹10,00,000 → ₹1,69,95,673 (+157.11% CAGR) over 2023-05-15 → 2026-05-12.
"""
...
def run(start: date, end: date, capital: float, out_dir: Path | None = None):
    ...
    for d in trading:
        ...
        # PIT universe: top-N by 20d ADV, then uptrend filter
        ...
        if not pit_univ:
            continue
        universe_log[d.date().isoformat()] = [
            s.replace("NSE:", "").replace("-EQ", "") for s in pit_univ
        ]
    ...
    if out_dir:
        ...
        (out_dir / "pit_universe_log.json").write_text(json.dumps(universe_log, indent=2))
        syms = sorted(set(t["sym"] for t in trades))
        (out_dir / "stocks_traded.json").write_text(json.dumps(syms, indent=2))
        print(f"  Wrote: {out_dir}/{{trade_ledger,pit_universe_log,stocks_traded}}.json")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=None, help="Output dir for JSON dumps")
    a = ap.parse_args()
    run(
