"""Live signal generator for regime_momentum_n500 (multi-holding, K=5).

Uses the SHARED core `strategy.py` (rank_targets / regime_healthy / hit_stop /
hit_trail / params) — the same logic the backtest uses — and the multi-holding
ledger (multi_holding_service).

Flow (runs daily; monthly-rebalance + daily-regime-risk, stateful via ledger):
  - DAILY (any session): if the market is in the BEAR regime (Nifty50 < 200DMA),
    check each holding's hard stop (-10% vs entry) and trailing stop (-15% off the
    peak since entry, recomputed from price history). Emit SELL on a hit.
  - MONTHLY (1st trading day): rank the top-K momentum leaders; emit SELL for any
    holding that dropped out of the top-RETAIN; emit BUY for the top-K not held.
...
log = logging.getLogger("regime_momentum_n500")
...
def main():
    ...
    # ---- DAILY bear-regime risk exits (stop / trail) ----
    if not healthy:
        for h in holdings:
            s = h["symbol"]
