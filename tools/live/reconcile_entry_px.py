...
def main() -> int:
    ...
    with db.get_session() as s:
        ...
        if not l.open_symbol or not l.open_qty:
            ...
            return 0
        bare = (l.open_symbol or "").upper().replace("NSE:", "").replace("-EQ", "")
        ...
        # Cash adjustment: ledger debited qty*old_px from cash at buy time.
        # Truth is qty*real_px. Diff = qty*(old_px - real_px). When real_px <
        # old_px (over-debited), credit cash by the diff so books match Fyers.
        cash_delta = float(old_qty) * (old_px - real_px)
        new_cash = old_cash + cash_delta

        log.info(f"=== Reconcile plan for {args.model} ===")
        log.info(f"  open_symbol      : {l.open_symbol}")
